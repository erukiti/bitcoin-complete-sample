...
describe('decode', () => {
  ...
  it('uint32', () => {
    expect(decodeUInt32LE(Buffer.from([255, 255, 255, 255])))
      .toEqual([4294967295, 4])
  })
  it('uint64LE', () => {
    expect(decodeUInt64LE(Buffer.from([0, 0, 0, 0, 0, 0, 0, 0])))
      .toEqual([0, 8])
    expect(decodeUInt64LE(Buffer.from([1, 0, 0, 0, 0, 0, 0, 0])))
      .toEqual([1, 8])
    expect(decodeUInt64LE(Buffer.from([255, 255, 255, 255, 0, 0, 0, 0])))
      .toEqual([4294967295, 8])
    expect(decodeUInt64LE(Buffer.from([0, 0, 0, 0, 1, 0, 0, 0])))
      .toEqual([4294967296, 8])
    expect(decodeUInt64LE(Buffer.from([255, 255, 255, 255, 255, 255, 31, 0])))
      .toEqual([0x1fffffffffffff, 8])
  })
})
...
describe('encode', () => {
  ...
  it('uint64', () => {
    expect(encodeUInt64LE(0))
      .toEqual(Buffer.from([0, 0, 0, 0, 0, 0, 0, 0]))
    expect(encodeUInt64LE(1))
      .toEqual(Buffer.from([1, 0, 0, 0, 0, 0, 0, 0]))
    expect(encodeUInt64LE(4294967295))
      .toEqual(Buffer.from([255, 255, 255, 255, 0, 0, 0, 0]))
    expect(encodeUInt64LE(4294967296))
      .toEqual(Buffer.from([0, 0, 0, 0, 1, 0, 0, 0]))
    expect(encodeUInt64LE(0x1fffffffffffff))
      .toEqual(Buffer.from([255, 255, 255, 255, 255, 255, 31, 0]))
  })
})
